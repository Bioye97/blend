...
int main(void)
{
    ...
    if (blend_polygon_alloc(&support, 8) != SUCCESS) {
        fprintf(stderr, "Could not allocate octagon vertices.\n");
        return FAIL;
    }

    blend_polygon_set_vertex(&support, 0, 35, 0);
    blend_polygon_set_vertex(&support, 1, 65, 0);
    blend_polygon_set_vertex(&support, 2, 99, 35);
    blend_polygon_set_vertex(&support, 3, 99, 65);
    blend_polygon_set_vertex(&support, 4, 65, 99);
    blend_polygon_set_vertex(&support, 5, 35, 99);
    blend_polygon_set_vertex(&support, 6, 0, 65);
    blend_polygon_set_vertex(&support, 7, 0, 35);

    if (blend_window_set_polygon(&data, &support) != SUCCESS) {
        fprintf(stderr, "Could not assign octagon vertices.\n");
