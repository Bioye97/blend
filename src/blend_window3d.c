...
static void window3d_usage(FILE *fp)
{
    ...
    fprintf(fp, "     and zlo/zhi define the vertical support slab. Grid points outside all\n");
    ...
    fprintf(fp, "          sampled on the -R/-I x/y grid.\n\n");
                                         ...
                                         window3d_support *support)
{
    ...
    if (!is_xy_monotone) {
        if (options->monotone_method != 'e' && options->monotone_method != 'b') {
            BLEND_Report(BLEND_MSG_ERROR,
                         "window3d: polygon %s is not xy-monotone; check -M if you want BLEND to refine it with -Me or -Mb\n",
                         polygon_file);
            blend_polygon_free(&input_polygon);
            return FAIL;
        }
        if (options->monotone_method == 'e') {
            BLEND_Report(BLEND_MSG_WARNING,
                         "window3d: polygon %s is not xy-monotone; using its xy-monotone envelope from -Me\n",
                         polygon_file);
        }
        else if (options->monotone_method == 'b') {
            BLEND_Report(BLEND_MSG_WARNING,
                         "window3d: polygon %s is not xy-monotone; using its best IoU piecewise xy-monotone envelope from -Mb\n",
                         polygon_file);
        }
    }
